...
import holidayFakerFactory from "./holidayFakerFactory";
...
const HolidayFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = holidayFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("holiday").create(elem));
...
export default connect(mapState, mapDispatch)(HolidayFakerDialogComponent);
